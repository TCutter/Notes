...
### XSS 攻击
XSS：Cross Site Scripting(跨站脚本攻击)是一种代码注入攻击。可以读取 cookie，session tokens，或者其它敏感的网站信息，对用户进行钓鱼欺诈。
...
#### 解决方法
1. 验证码（体验不好）
2. [Token](https://www.cnblogs.com/moyand/p/9047978.html)
3. 接口设置禁止跨域
4. 设置白名单
...
4. 返回这个对象(如果构造函数有自己 `return` 时，则返回该值)
...
而异步队列分为两种：
- `microtask`: 微任务，优先级高，并且可以插队，不是先定义先执行。包括：`promise` 中的 `then`，`observer`，`MutationObserver`
- `macrotask`: 宏任务，优先级低，先定义的先执行。包括：`ajax`，`setTimeout`，`setInterval`，`setImmediate`，事件绑定，`postMessage`，`MessageChannel`（用于消息通讯）
...
- 触发：任何对元素样式，如 `background-color、border-color、visibility` 等属性的改变。css 和 js 都可能引起重绘

#### 回流
- 概念：元素的大小、位置发生了改变，而导致了布局的变化，从而导致了布局树的重新构建和渲染。
- 触发
    - dom元素的位置和尺寸大小的变化
    - dom元素的增加和删除
    - 伪类的激活
...
### 存储
- `cookie` ：通常用于存储用户身份，登录状态
    - http 中自动携带， 体积上限为 4K， 可自行设置过期时间
- `localStorage` : 永久存储，关闭浏览器也不会消失，不能跨域使用，上限 5M 左右
- `sessionStorage`: 关闭浏览器后清空

### 垃圾回收
定义： 将不被使用的数据清理，释放出内存空间。b包括新生代空间和老生代空间两种

#### 新生代空间
用于存活较短的对象
- 分为两种 `from` 空间和 `to`空间
- GC: 当 `from` 空间被占满时，启动 `GC`
    - 将 `from` 中活动的对象转移到 `to`
    - 清空 `from`
    - 将 `from` 与 `to` 互换
    - 完成
...
- 客户端接收 `syn+ack` 包后，发送 `ack` 包，双方建立连接，此时可以客户端可以发送数据
